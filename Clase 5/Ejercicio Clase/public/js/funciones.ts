...
namespace AJAX {
    ...
    xhttp.onreadystatechange = () => {
      if (xhttp.readyState == 4 && xhttp.status == 200) {
        ...
        switch (lista) {
          ...
          case 2:
            let divJson = <HTMLDivElement>(
              document.getElementById("div_listadoJson")
            );
            divJson.innerText = alumnosArrayStr;
            break;
  ...
  export function VerificarAlumno(): boolean {
    let retorno = false;
    let legajo: number = parseInt(
      (<HTMLInputElement>document.getElementById("txtLegajoVer")).value
    );
    let alumno = {
      legajo: legajo,
    };
    ...
    xhttp.onreadystatechange = () => {
      if (xhttp.readyState == 4 && xhttp.status == 200) {
        ...
        if (alumnoJson.legajo != undefined) {
          ...
          console.log(
            `El alumno con legajo ${alumno.legajo} no esta en el listado.`
          );
        }
      }
    };
  ...
  export function ModificarAlumno() {
    let legajo: number = parseInt(
      (<HTMLInputElement>document.getElementById("txtLegajoMod")).value
    );
    let nombre: string = (<HTMLInputElement>(
  ...
  export function BorrarAlumno() {
    let legajo: number = parseInt(
      (<HTMLInputElement>document.getElementById("txtLegajoDel")).value
    );
    let alumno = {
      legajo: legajo,
    };
